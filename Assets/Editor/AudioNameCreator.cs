...
public static class AudioNameCreator
{
    ...
    private static readonly string FILENAME = Path.GetDirectoryName(EXPORT_PATH);
    ...
    public static void CreateScript()
    {
        ...
        {
            builder.Append("\t").AppendFormat(@"    public const string BGM_{0} = ""{1}"";"
            , bgm.name.ToUpper()
            , bgm.name).AppendLine();
        }
        builder.AppendLine("\t");
        ...
        {
            builder.Append("\t").AppendFormat(@"    public const string SE_{0} = ""{1}"";"
            , se.name.ToUpper()
            , se.name).AppendLine();
        }
        builder.AppendLine("}");
